...
def config_parser():
    ...
    parser.add_argument("--encoding_dim_view", type=int, default=10, 
                        help='the dimmension of positional encoding, also L in the paper, attention that R is mapped to R^2L')    
    parser.add_argument("--skips", type=int, default=[], action='append',
                        help='the dimmension of positional encoding, also L in the paper, attention that R is mapped to R^2L')   
...
def train():
    ...
    if args.use_encoding:
        model = Network(D = args.layer_nums, H = args.hiddenlayer_dim, input_ch = 6 * args.encoding_dim, input_ch_views = 4 * args.encoding_dim_view, skips=args.skips, no_rho=args.no_rho)
    else:
        model = Network(D = args.layer_nums, H = args.hiddenlayer_dim, input_ch = 6 * args.encoding_dim, input_ch_views = 4 * args.encoding_dim_view, skips=args.skips, no_rho=args.no_rho)
    ...
    for i in trange(0, N_iters):
        ...
        input_coord, _, theta, phi, _, _, r_batch = spherical_sample_bin_tensor(camera_grid_positions[:,index_rand], r[index_rand].squeeze(), args.sampling_points_nums)
        input_hist = torch.cat((input_coord, theta, phi), dim=1)
        sin_theta = torch.sin(theta)
        input_hist = (input_hist - pmin) / (pmax - pmin)
        if args.use_encoding:
            input_hist = encoding_sph_tensor(input_hist, args.encoding_dim, args.encoding_dim_view, args.no_rho)
        
        # predict transient
        network_res = model(input_hist)
        if not args.no_rho:
            network_res = torch.mul(network_res[:,0], network_res[:,1])

        network_res = torch.mul(network_res.reshape([-1,1]), sin_theta)
        ...
        if (i+1) % i_image == 0:
            with torch.no_grad():
                ...
                plt.close()
                io.savemat(result_path + 'vol_' + str(i+1) + '.mat' , {'res_vol': output.reshape([reso, reso, reso,2]).cpu().data.numpy().squeeze()})
