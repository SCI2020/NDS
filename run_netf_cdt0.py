...
def train():
    ...
    nlos_data0 = torch.Tensor(nlos_data).reshape([Nz, Nx, Ny])
    ...
    for i in trange(0, N_iters):
        ...
        cdt_pad = torch.clone(cdt_conv.squeeze())
        cdt_pad[:Nz,:Nx,:Ny] = nlos_data0
        
        if args.shift:
            cdt_pad_fft = torch.fft.fftn(cdt_pad, s=(Nz*2, Nx*2-1, Ny*2-1))                  
        else:
            cdt_pad_fft = torch.fft.fftn(cdt_pad, s=(Nz*2, Nx*2, Ny*2))
        target_nlos = torch.fft.ifftn(torch.mul(cdt_pad_fft, invpsf)).real.squeeze()[:Nz,:Nx,:Ny]
        loss = criterion(nlos_pad.squeeze()[target_nlos>=0][:-Nx*Ny*Nz//10], target_nlos[target_nlos>=0][:-Nx*Ny*Nz//10])
        ...
        if (i+1) % i_hist == 0:
            ...
            plt.plot(nlos_pad.squeeze()[target_nlos>=0][:-Nx*Ny*Nz//10].cpu().detach().numpy().flatten(), alpha = 0.5, label = 'nlos')
            plt.plot(target_nlos[target_nlos>=0][:-Nx*Ny*Nz//10].cpu().detach().numpy().flatten(), alpha = 0.5, label = 'target_nlos')   
