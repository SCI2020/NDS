...
def train():
    ...
    for i in trange(0, N_iters):
        ...
        # time0 = time.time()
        # print(camera_grid_positions[:,index_rand].shape, r[index_rand].squeeze().shape )
        # return
        # input_coord1, input_dir1, theta, _, _, _, r_batch = spherical_sample_bin_tensor(camera_grid_positions[:,index_rand], r[index_rand].squeeze(), args.sampling_points_nums)
        # print(f"Sampling: {input_coord1.shape, input_dir1.shape, input_coord1.dtype}")
        # return
        input_coord, input_dir = ncrelu_cuda.ncrelu_forward_cuda(camera_grid_positions[:,index_rand], r[index_rand].squeeze(), args.sampling_points_nums)
        input_coord = input_coord.reshape([3,-1])
        input_dir = input_dir.reshape([3,-1])
        input_coord = input_coord.permute([1,0])
        input_dir = input_dir.permute([1,0])        
        # print(f"Sampling: {input_coord.shape, input_dir.shape, input_coord.dtype}")
        # print(f"Sampling: {time.time()-time0}")
        # diff = input_coord1 - input_coord
        ...
        # input_coord = (input_coord - pmin) / (pmax - pmin)
        # input_dir = (input_dir - pmin_dir) / (pmax_dir - pmin_dir)
        # print(f'input_coord: {input_coord.max(), input_coord.min()}. input_dir: {input_dir.max(), input_dir.min()}')
        ...
        # print(f'network_res: {network_res.max(), network_res.min()}, nlos_histogram: {nlos_histogram.max(), nlos_histogram.min()}')
